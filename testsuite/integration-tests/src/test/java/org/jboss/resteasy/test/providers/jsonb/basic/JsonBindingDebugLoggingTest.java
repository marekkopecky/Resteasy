...
public class JsonBindingDebugLoggingTest {
   ...
   public void exceptionDuringServerSend() throws Exception {
      ...
      LogCounter errorStringLog = new LogCounter("ERROR", false, ContainerConstants.DEFAULT_CONTAINER_QUALIFIER);

      // TBD?: s/ERROR/DEBUG/g
      // TBD?: stacktraces are printed twice currently. RESTEASY002025 is first WF exception.
      LogCounter resteasyExceptionLog = new LogCounter(".*ERROR .* RESTEASY002025.*", true, ContainerConstants.DEFAULT_CONTAINER_QUALIFIER, true);
      ...
      LogCounter applicationExcpetionLog = new LogCounter("Caused by: java.lang.RuntimeException: "
            + JsonBindingDebugLoggingItemCorruptedGet.class.getSimpleName(), true, ContainerConstants.DEFAULT_CONTAINER_QUALIFIER);
      ...
      Assert.assertThat("Response message doesn't contains full stacktrace", response.readEntity(String.class), allOf(
            containsString("at org.eclipse.yasson.internal"),
            containsString("java.lang.RuntimeException: " + JsonBindingDebugLoggingItemCorruptedGet.class.getSimpleName()),
      ...
      Assert.assertThat("Application Exception should be logged", applicationExcpetionLog.count(), is(2)); // TBD replace to 1
      Assert.assertThat("RESTEasy exception should be logged", resteasyExceptionLog.count(), is(1));
      Assert.assertThat("Yasson exception should be logged", yassonExceptionLog.count(), is(2)); // TBD replace to 1
      Assert.assertThat("Yasson exception stacktrace should be logged", yassonStacktraceLog.count(), greaterThan(0));

      // TBD: uncomment following line?
      // Assert.assertThat("There shouldn't be any error logs in server", errorStringLog.count(), is(0));
   }
   ...
   public void exceptionDuringServerReceive() throws Exception {
      ...
      // TBD?: s/ERROR/DEBUG/g
      // TBD: stacktraces are not printed at all
      LogCounter resteasyExceptionLog = new LogCounter(".*ERROR .* RESTEASYTBD.*", true, ContainerConstants.DEFAULT_CONTAINER_QUALIFIER, true);
      ...
      wrongItem.setA(5);
      Response response = base.request().post(Entity.entity(wrongItem, MediaType.APPLICATION_JSON));

      // check response
      Assert.assertThat("Wrong response code", response.getStatus(), is(500));
      Assert.assertThat("Response message doesn't contains full stacktrace", response.readEntity(String.class), allOf(
            containsString("at org.eclipse.yasson.internal"),
            containsString("java.lang.RuntimeException: " + JsonBindingDebugLoggingItemCorruptedSet.class.getSimpleName()),
            containsString("javax.json.bind.JsonbException: TBD"),
            containsString("RESTEASYTBD")
      ));

      // assert log messages after request
      Assert.assertThat("Application Exception should be logged", applicationExcpetionLog.count(), is(1)); // TBD replace to 1
      Assert.assertThat("RESTEasy exception should be logged", resteasyExceptionLog.count(), is(1));
      Assert.assertThat("Yasson exception should be logged", yassonExceptionLog.count(), is(2)); // TBD replace to 1
      Assert.assertThat("Yasson exception stacktrace should be logged", yassonStacktraceLog.count(), greaterThan(0));
   ...
   public void exceptionDuringClientReceive() throws Exception {
      ...
      // server logging should be done for this request, we can start monitor client logging
      // TBD check debug logs? Or exception itself is sufficient?
      ...
      try {
         ...
         Assert.assertThat("Stracktrace doesn't contain application exception", stackTrace,
               containsString("Caused by: java.lang.RuntimeException: " + JsonBindingDebugLoggingItemCorruptedSet.class.getSimpleName()));
      }

      // assert log messages after request
      Assert.assertThat("Application Exception should be logged", applicationExcpetionLog.count(), is(1));
      Assert.assertThat("RESTEasy exception should be logged", resteasyExceptionLog.count(), is(1));
      Assert.assertThat("Yasson exception should be logged", yassonExceptionLog.count(), is(1));
      Assert.assertThat("Yasson exception stacktrace should be logged", yassonStacktraceLog.count(), greaterThan(0));

      // TBD: uncomment following line?
      // Assert.assertThat("There shouldn't be any error logs in client", errorStringLog.count(), is(0));
   }
   ...
   public void exceptionDuringClientSend() throws Exception {
      ...
      try {
         ...
         Assert.assertThat("Stracktrace doesn't contain yasson part", stackTrace,
               containsString("org.eclipse.yasson.internal"));
         Assert.assertThat("Stracktrace doesn't contain application exception", stackTrace,
               containsString("Caused by: java.lang.RuntimeException: " + JsonBindingDebugLoggingItemCorruptedGet.class.getSimpleName()));
      }
